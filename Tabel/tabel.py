...
fig = go.Figure(data=[go.Table(
    header=dict(values=["Posisi", "Instansi", "Gaji", "Lokasi", "Median Gaji"]),
    cells=dict(values=[lowkers_df['Posisi'], lowkers_df['Instansi'], lowkers_df['Gaji'], lowkers_df['Lokasi'], [median_gaji] * len(lowkers_df)]))
])

fig.update_layout(
    title="Tabel Lowongan Kerja",
)
